...
typedef (int)(*)(void* , void*) cb_db_search;

dll_t *
get_new_dll(cb_db_search key_match_fn){
...
dll_node_t * search_entry_in_dll(dll_t *dll, void* key){
    if(!dll || !key ) return -1;

    dll_node_t *temp = dll->head;

    for(;temp != NULL; ){
        if(dll->key_match(dll_node->data, key))
            return temp;
    }
    return NULL;
