...
match_student_rec_by_key(void *data, void *key){
    mydata = (student_t*)data;

    if(mydata->rollno = *(unsigned int)key)
        return 1;
    else
        return 0;
}
...
int
match_emp_rec_by_key(void *data, void *key){
    mydata = (employee_t*)data;

    if(mydata->emp_id = *(unsigned int *)key)
        return 1;
    else
        return 0;
}
...
main(int argc, char **argv){
    ...
    dll_t *student_db = get_new_dll(search_student_by_rollno);
...
//student_t *student = search_student_by_rollno(student_db, 800400);
    roll_no = 800400;
    student_t *student = match_emp_rec_by_key(employee_db, &roll_no);
    if(!student){
    ...
    dll_t *employee_db = get_new_dll(search_employee_by_emp_id);
...
//    employee_t *employee = search_employee_by_emp_id(employee_db, 10002);
    ...
    employee_id = 10002;
    employee_t *employee = match_emp_rec_by_key(employee_db, &employee_id);
    if(!employee){
        printf("Student record not found\n");
    }
    else{
